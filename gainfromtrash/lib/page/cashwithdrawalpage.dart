...
class CashWithdrawalPage extends StatefulWidget {
  final User user;
  const CashWithdrawalPage({super.key, required this.user});
...
class _CashWithdrawalPageState extends State<CashWithdrawalPage> {
  ...
  void confirmWithdraw() {
    ...
    double availableBalance = double.parse(widget.user.balance.toString());
  ...
  void cashWithdraw(){
    ...
    try {
      http.post(Uri.parse("${Config.server}/gainfromtrash/php/cash_withdraw.php"), body: {
